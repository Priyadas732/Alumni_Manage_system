...
export default function RoleSelectionProfileSetup() {
  ...
  const handleCompleteSetup = (e) => {
    ...
    if (role === 'student') {
      ...
      const preferences = {
        availableForReferrals: formData.openToReferrals,
        availableForMentorship: formData.openToMentoring
      };
      localStorage.setItem('userRole', 'alumni');
      localStorage.setItem('userProfile', JSON.stringify(profileData));
      localStorage.setItem('alumniPreferences', JSON.stringify(preferences));
    }
    navigate('/hub');
  };
          ...
          {step === 1 && (
            <section className="p-xl flex flex-col gap-xl">
              <header className="text-center mb-md animate-fade-in">
                ...
                <h1 className="font-headline-lg text-headline-lg text-on-surface mb-sm">Welcome to Ember & Ash</h1>
